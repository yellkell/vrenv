...
function buildCatwalk(env: Group): void {
  ...
  // Decks: north, east, west (entrance side stays open).
  place(env, box(h * 2, 0.18, depth, deck), 0, y, -h + depth / 2);
  place(env, box(depth, 0.18, h * 2, deck), h - depth / 2, y, 0);
  place(env, box(depth, 0.18, h * 2, deck), -h + depth / 2, y, 0);

  // Inner handrails along the open edge of each deck.
  const rN = handrail(h * 2);
  place(env, rN, 0, y + 0.09, -h + depth);
  const rE = handrail(h * 2);
  rE.rotation.y = Math.PI / 2;
  place(env, rE, h - depth, y + 0.09, 0);
  const rW = handrail(h * 2);
  rW.rotation.y = Math.PI / 2;
  place(env, rW, -h + depth, y + 0.09, 0);
